...
from app.utilities.crud import get_categories_for_trader, new_catagory
...
async def get_category(id: str, user=Depends(Authentication().validate_token), db: AsyncSession = Depends(get_db)) -> dict:
    categories = await get_categories_for_trader(db, id)
    if categories is None:
        raise HTTPException(status_code=404, detail="No categories found")
    return {"categories": categories}

@category_route.post("/new_category")
async def new_category(category: Category, user=Depends(Authentication().validate_token), db: AsyncSession = Depends(get_db)) -> dict:
