...
from app.utilities.crud import get_trader_by_id, get_trader_by_url 
from app.utilities.authentication import Authentication
from app.validation_models import TraderModel, TraderURLModel
...
@trader_route.get("/check_trader_url", response_model=TraderModel)
async def check_trader_url(urlmodel: TraderURLModel, db: AsyncSession = Depends(get_db)) -> dict:
    trader = await get_trader_by_url(db, urlmodel.url)
    if trader is None:
        raise HTTPException(status_code=404, detail="Trader not found")
    return trader
